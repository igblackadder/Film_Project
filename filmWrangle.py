...
#import data
def get_data():
    '''Returns a pandas data frame of the wrangled data. 
        The wrangling consists of 
            ...
            4) convert the column 'Wri/DirOverlap' into two columns of 1s and 0s that
                designate whether a film had a writer who was also a director.
                Overlap = 1 when the film has a writer who was also a director
                Overlap = 0 when the film does NOT have a writer who was also a director
                nonOverlap = 1 when the film does NOT have a writer who was also a director
                nonOverlap = 0 when the film has a writer who was also a director
...
def add_cols(df, fromCol, newCols, prefix):
    '''Instead of giving a list of countries, languages and genres associated 
        ...
        language and genre. 1 means the film was made in the corresponding country, in the corresponding language or within the corresponding genre. 0 otherwise
        ...
        '''
    for col in newCols:
        tmp = lambda x: int(col in split(x, ', '))
        df[prefix+col] = df[fromCol].map(tmp)
...
if __name__ == '__main__':
    ...
    countries = get_unique(raw, "country")#research and munge
    languages = get_unique(raw, "language")#research and munge
    genres = get_unique(raw, "genre")

    print "adding country columns"
    add_cols(raw, 'country', countries, u'Cou_')
    print "adding language columns"
    add_cols(raw, 'language', languages, u'Lan_')
    print "adding genre columns"
    add_cols(raw, 'genre', genres, u'Gen_')
